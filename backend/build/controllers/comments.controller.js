...
export const likeComment = async (req, res) => {
    try {
        ...
        const user = res.locals.user;
        const _ = await pool.query('INSERT INTO comments_likes(user_id, comment_id) VALUES($1, $2) RETURNING * ', [user.user_id, comment.comment_id]);
        const __ = pool.query(`UPDATE comments SET num_likes = $1 WHERE comment_id = $2`, [comment.num_likes + 1, comment.comment_id]);
        return res.status(201).send({
...
export const editComment = async (req, res) => {
    try {
        ...
        const results = await pool.query(`UPDATE comments SET text = $1 WHERE comment_id = $2`, [newComment, comment.comment_id]);
        return res
            .status(200)
