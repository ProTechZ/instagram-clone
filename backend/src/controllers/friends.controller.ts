...
export const followUser = async (req: Request, res: Response) => {
  try {
    ...
    pool.query(
      'SELECT * FROM followed_following WHERE user_followed_id = $1 AND user_following_id = $2',
      [userToFollow, userId],
      (err, results) => {
        if (err) {
          return res.send({ err });
          ...
          pool.query(
            'INSERT INTO followed_following(user_followed_id, user_following_id) VALUES($1, $2) RETURNING *',
            [userToFollow, userId],
            (err, results) => {
              if (err) {
                res.send({ err });
    ...
    pool.query(
      ...
      (err, results) => {
        if (err) {
          ...
          pool.query(
            'DELETE FROM followed_following WHERE user_followed_id = $1 AND user_following_id = $2 RETURNING *',
            [followedUser, followingUser],
            (err, results) => {
              if (err) {
                return res.send({ err });
...
export const getAllFollowers = async (req: Request, res: Response) => {
  try {
    ...
    pool.query(
      `SELECT * FROM followed_following WHERE user_followed_id = $1`,
      [userId],
      (err, results) => {
        if (err) {
          return res.send({ err });
